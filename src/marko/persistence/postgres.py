...
class PostgresStore:
    ...
    def activities(self, as_of: datetime | None = None) -> tuple[Activity, ...]:
        ...
        ledger = Ledger(
            decode_activity(SerializationEnvelope.from_json(row)) for row in rows
        )
    ...
    def append_decision_packet(self, packet: DecisionPacket) -> None:
        ...
        self._append(
            """
            INSERT INTO marko_decision_packets (packet_id, created_at, payload, payload_hash)
            VALUES (%s, %s, %s::jsonb, %s)
            ON CONFLICT (packet_id) DO NOTHING
            """,
            (packet.packet_id, packet.created_at, envelope.canonical_json(), envelope.digest()),
    ...
    def get_decision_packet(self, packet_id: str) -> DecisionPacket:
        return decode_decision_packet(
            self._get("marko_decision_packets", "packet_id", packet_id)
        )
    ...
    def decision_packets(self) -> tuple[DecisionPacket, ...]:
        ...
        return tuple(
            decode_decision_packet(SerializationEnvelope.from_json(row)) for row in rows
        )
