...
def loan() -> Liability:
    return Liability(
        liability_id="loan-mother",
        creditor="Mãe",
        initial_principal=Money.of("50000", "BRL"),
        originated_on=date(2026, 8, 20),
        maturity_date=date(2030, 8, 20),
        cashflows=(
            LiabilityCashflow(
                "principal", date(2030, 8, 20), Money.of("50000", "BRL"), "Principal"
            ),
        ),
        callable_early=True,
    )
...
def test_liability_remains_outstanding_until_settlement() -> None:
    assert loan().outstanding_on(date(2031, 1, 1)) == Money.of("50000", "BRL")


def test_funding_ratio_and_shortfall() -> None:
    assets = Money.of("42000", "BRL")
    obligation = loan().outstanding_on(date(2028, 1, 1))
    assert funding_ratio(assets, obligation) == Decimal("0.84")
    assert shortfall(assets, obligation) == Money.of("8000", "BRL")
    assert shortfall(Money.of("60000", "BRL"), obligation) == Money.zero("BRL")
